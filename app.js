// Dados dos ADRs baseados no JSON fornecido
...
// Cache de elementos DOM otimizado para melhor performance
...
function initializeLazyLoading() {
    ...
    // Observar elementos com loading="lazy"
...
function calculateRealSpreadAnalysis(ticker, closingPrice, afterPercentage, prePercentage) {
    ...
    return {
        ...
        totalMovement: Math.max(0.01, Math.abs(preSpread)), // ✅ CORRIGIDO: Movimento líquido real
        volatilityRange: Math.max(0.01, Math.abs(afterSpread) + Math.abs(afterToPreSpread)), // ✅ NOVO: Amplitude de volatilidade
        direction: {
...
function determineAdvancedValidationType(spreadAnalysis) {
    ...
    // NOVO: Verificação de soma zero - usar valores originais dos inputs
    const afterInput = document.getElementById(`after-${spreadAnalysis.ticker}`);
    const preInput = document.getElementById(`pre-${spreadAnalysis.ticker}`);
    
    if (afterInput && preInput) {
        const afterValue = parseFloat(afterInput.value) || 0;
        const preValue = parseFloat(preInput.value) || 0;
        const sum = afterValue + preValue;
        
        if (sum === 0 && afterValue !== 0 && preValue !== 0) {
            return {
                type: 'zero_sum',
                icon: '🔄',
                status: 'Soma Zero - Neutro',
                description: `Movimentos opostos: ${afterValue > 0 ? '+' : ''}${afterValue.toFixed(2)}% + ${preValue > 0 ? '+' : ''}${preValue.toFixed(2)}% = 0%`,
                confidence: 50,
                recommendation: 'Movimentos opostos se cancelam - Sinal neutro',
                color: COLOR_SCHEME.NEUTRAL
            };
        }
    }
    ...
    if ((!afterSpread || afterSpread === 0) && (!preSpread || preSpread === 0)) {
        // CORREÇÃO: Obter a variação percentual do campo "Fechamento (%)" da interface
        const closingInput = document.getElementById(`closing-${spreadAnalysis.ticker}`);
        const closingSpread = closingInput ? (parseFloat(closingInput.value) || 0) : 0;
        
        console.log(`📊 Análise apenas fechamento ${spreadAnalysis.ticker}: ${closingSpread}%`);
        console.log(`🔍 Debug - afterSpread: ${afterSpread}, preSpread: ${preSpread}, closingSpread: ${closingSpread}`);
    ...
    // Caso 3: After + Pré-Market disponíveis (pode incluir dados de fechamento)
...
function analyzeClosingOnly(closingSpread) {
    ...
    // Caso especial: sem dados de fechamento
    if (absClosingSpread === 0) {
        return {
            type: 'closing_no_data',
            icon: '⚪',
            status: 'Apenas Fechamento - Sem Dados',
            description: 'Variação: 0,0%',
            confidence: 10,
            recommendation: 'Inserir variação de fechamento',
            color: COLOR_SCHEME.NEUTRAL
        };
    }
    
    // Movimento muito pequeno (< 0.1%)
    if (absClosingSpread < 0.1) {
        return {
            type: 'closing_neutral',
            icon: '⚪',
            status: 'Apenas Fechamento - Neutro',
            description: `Mínimo: ${closingSpread > 0 ? '+' : ''}${closingSpread.toFixed(2)}%`,
            confidence: 30,
            recommendation: 'Movimento insignificante - Aguardar dados After/Pré',
            color: COLOR_SCHEME.NEUTRAL
        };
    }
    
    // Movimento fraco (0.1% - 1.0%)
    if (absClosingSpread < 1.0) {
        return {
            type: 'closing_weak',
            icon: closingSpread > 0 ? '🟢' : '🔴',
            status: `Apenas Fechamento - ${closingSpread > 0 ? 'Positivo' : 'Negativo'} Fraco`,
            description: `Moderado: ${closingSpread > 0 ? '+' : ''}${closingSpread.toFixed(2)}%`,
            confidence: 40,
            recommendation: closingSpread > 0 ? 'Leve viés de alta - Confirmar com After/Pré' : 'Leve viés de baixa - Confirmar com After/Pré',
    ...
    return {
        ...
        description: `Significativo: ${closingSpread > 0 ? '+' : ''}${closingSpread.toFixed(2)}%`,
        confidence: 50,
        recommendation: closingSpread > 0 ? 'Forte viés de alta - Validar com After/Pré-Market' : 'Forte viés de baixa - Validar com After/Pré-Market',
        color: getMovementColor(closingSpread)
    };
}
...
function analyzeAfterAndPreMarket(afterSpread, preSpread, afterToPreSpread, direction, totalMovement) {
    // NOVO: Verificação geral - Soma igual a zero (positivo + negativo = 0)
    const sum = afterSpread + preSpread;
    if (sum === 0) {
        return {
            type: 'zero_sum',
            icon: '🔄',
            status: 'Soma Zero - Neutro',
            description: `Movimentos opostos: ${afterSpread > 0 ? '+' : ''}${afterSpread.toFixed(2)}% + ${preSpread > 0 ? '+' : ''}${preSpread.toFixed(2)}% = 0%`,
            confidence: 50,
            recommendation: 'Movimentos opostos se cancelam - Sinal neutro',
            color: COLOR_SCHEME.NEUTRAL
        };
    ...
    // NOVO: Caso especial - Movimento de "ida e volta" (resultado quase neutro)
    const isRoundTrip = Math.abs(preSpread) < 0.1 && (Math.abs(afterSpread) > 1.0 || Math.abs(afterToPreSpread) > 1.0);
    if (isRoundTrip) {
        return {
            type: 'round_trip_neutral',
            icon: '🔄',
            status: 'Movimento Circular - Quase Neutro',
            description: `Ida: ${afterSpread > 0 ? '+' : ''}${afterSpread.toFixed(2)}%, Volta: ${afterToPreSpread > 0 ? '+' : ''}${afterToPreSpread.toFixed(2)}% (Líquido: ${preSpread > 0 ? '+' : ''}${preSpread.toFixed(2)}%)`,
            confidence: 25,
            recommendation: 'Alta volatilidade intraday com resultado neutro - Aguardar confirmação',
            color: COLOR_SCHEME.NEUTRAL
        };
    }
...
function analyzeSameDirection(afterSpread, preSpread, afterToPreSpread, direction, totalMovement) {
    ...
    // NOVO: Verificação específica para soma igual a zero
    const sum = afterSpread + preSpread;
    if (sum === 0) {
        return {
            type: 'zero_sum_same_direction',
            icon: '🔄',
            status: 'Soma Zero - Mesma Direção',
            description: `Movimentos opostos: ${afterSpread > 0 ? '+' : ''}${afterSpread.toFixed(2)}% + ${preSpread > 0 ? '+' : ''}${preSpread.toFixed(2)}% = 0%`,
            confidence: 60,
            recommendation: 'Movimentos opostos se cancelam - Sinal neutro',
            color: COLOR_SCHEME.NEUTRAL
...
// ===== REGIME DE VOLATILIDADE ADAPTATIVO =====

/**
 * Calcula a volatilidade dos ADRs baseada nos spreads
 * @returns {number} Volatilidade dos ADRs (desvio padrão dos volatilityRange)
 */
function calculateADRsVolatility() {
    ...
    adrsData.principais.forEach(adr => {
        ...
        if (spreadAnalysis && spreadAnalysis.volatilityRange !== undefined) {
            console.log(`  ✅ ${adr.ticker}: volatilityRange = ${spreadAnalysis.volatilityRange.toFixed(2)}`);
            spreads.push(spreadAnalysis.volatilityRange);
        } else {
...
function updateVolatilityRegime() {
    ...
    // Determinar regime baseado em VIX e volatilidade dos ADRs
    if (vixLevel > 50) {
...
function updateVolatilityRegimeInterface() {
    ...
    // Atualizar métricas do Timing Effect
    ...
    return `
        <div class="spread-card">
            ...
            <div class="spread-metrics">
                ...
                <div class="metric-row">
                    <span class="metric-label">Movimento Líquido:</span>
                    <span class="metric-value ${spreadAnalysis.preSpread > 0 ? 'positive' : 'negative'}">
                        ${spreadAnalysis.preSpread > 0 ? '+' : ''}${spreadAnalysis.totalMovement.toFixed(2)}%
                    </span>
                </div>
                <div class="metric-row">
                    <span class="metric-label">Volatilidade Intraday:</span>
                    <span class="metric-value neutral">${spreadAnalysis.volatilityRange.toFixed(2)}%</span>
                </div>
                ` : `
                <div class="metric-row">
                    <span class="metric-label">Movimento Líquido:</span>
                    <span class="metric-value ${spreadAnalysis.afterSpread > 0 ? 'positive' : 'negative'}">
                        ${spreadAnalysis.afterSpread > 0 ? '+' : ''}${Math.abs(spreadAnalysis.afterSpread).toFixed(2)}%
                    </span>
                </div>
                <div class="metric-row">
                    <span class="metric-label">Volatilidade Intraday:</span>
                    <span class="metric-value neutral">${Math.abs(spreadAnalysis.afterSpread).toFixed(2)}%</span>
                </div>
                `}
            </div>
...
function initializeApp() {
    ...
    // Inicializar ScrollReveal para os cards de cotações
...
// Night Mode Functions - Always apply dark mode
...
function updateAllCalculations() {
    ...
    // Atualizar Saldo Remanescente ADRs After Market
    updateSaldoRemanescenteInterface();
...
// ============================================================================
// SALDO REMANESCENTE ADRs AFTER MARKET - Metodologia Mari Barros + Deivson Pimentel
// ============================================================================

function calculateSaldoRemanescenteADRs() {
    ...
    return {
        ...
        adrsComDados: adrsComDados
    };
}

function updateSaldoRemanescenteInterface() {
    const saldoRemanescenteElement = document.getElementById('saldoRemanescenteADRs');
    if (!saldoRemanescenteElement) return;
    
    const resultado = calculateSaldoRemanescenteADRs();
    const saldo = resultado.saldo;
    const adrsComDados = resultado.adrsComDados;
    
    // Formatar o valor (usando a mesma formatação do sistema)
    const saldoFormatado = formatPercentage(saldo, CONFIG.FORMAT.PERCENTAGE_DECIMALS);
    saldoRemanescenteElement.textContent = saldoFormatado + '%';
    
    // Aplicar cores (usando o mesmo sistema de cores)
    saldoRemanescenteElement.classList.remove('positive', 'negative');
    if (saldo > 0) {
        saldoRemanescenteElement.classList.add('positive');
    } else if (saldo < 0) {
        saldoRemanescenteElement.classList.add('negative');
    }
    
    // Atualizar confiança
    const confidenceElement = saldoRemanescenteElement.parentElement.querySelector('.breakdown-confidence');
    if (confidenceElement) {
        confidenceElement.textContent = `(${adrsComDados} ADRs com dados)`;
    }
}
    ...
    updateADRsData(adrsData) {
        ...
        Object.keys(adrsData).forEach(ticker => {
            ...
            if (data.data_type === 'closing' || data.source === 'closing-price') {
                const closingInput = document.getElementById(`closing-${ticker}`);
                if (closingInput && data.variation !== null && data.variation !== undefined) {
                    populateVariationInput(closingInput, data.variation);
                }
            }

            if (data.after_market) {
                const afterInput = document.getElementById(`after-${ticker}`);
                populateVariationInput(afterInput, data.after_market.percent);
            }

            if (data.pre_market) {
                const preInput = document.getElementById(`pre-${ticker}`);
                const prePercent = data.pre_market.percent_incremental !== null && data.pre_market.percent_incremental !== undefined
                    ? data.pre_market.percent_incremental
                    : data.pre_market.percent_total;
                populateVariationInput(preInput, prePercent);
            }
        });
    }
   ...
   constructor() {
    ...
    updateQuoteCard(type, price, change, source = 'regular-market', hasAfterMarket = true) {
        ...
        if (cardElement) {
            ...
            // Aplicar classes de cor baseadas na variação no CARD PRINCIPAL
            // Remover classes anteriores
            ...
            if (change !== null && change !== undefined && !isNaN(change)) {
                if (change > 0) {
                    ...
                    console.log(`✅ Aplicando classe positive no CARD ${type}: ${change}%`);
                } else if (change < 0) {
                    cardElement.classList.add('negative');
                    console.log(`✅ Aplicando classe negative no CARD ${type}: ${change}%`);
                } else {
                    cardElement.classList.add('neutral');
                    console.log(`✅ Aplicando classe neutral no CARD ${type}: ${change}%`);
                }
            } else {
                cardElement.classList.add('no-data');
                console.log(`✅ Aplicando classe no-data no CARD ${type}: sem dados`);
            }
...
class WinfutProjectionService {
    ...
    updateCalculatorVariation(variation, confidence) {
        ...
        this.calculatorVariation = variation; // ✅ REVERTIDO: Manter como estava (mais realista)
        ...
        // Integração com sistema adicional (não interfere no funcionamento principal)
        if (typeof winfutDualProjection !== 'undefined') {
            winfutDualProjection.onMainSystemUpdate();
        }
    }
...
// ===== EXTENSÃO ADICIONAL: SISTEMA DE DUPLA PROJEÇÃO WINFUT =====
// Esta é uma extensão ADICIONAL que não interfere no sistema principal
class WinfutDualProjectionExtension {
    constructor() {
        this.isEnabled = false;
        this.dualProjectionData = null;
        console.log('🆕 Sistema de Dupla Projeção WINFUT carregado (extensão adicional)');
    }
    
    
    // Função ADICIONAL: Calcular dupla projeção (não interfere na principal)
    calculateDualProjection() {
        if (!this.isEnabled) return;
        
        // Obter variação do sistema principal (sem modificá-lo)
        const mainVariation = winfutProjectionService.calculatorVariation;
        const mainConfidence = winfutProjectionService.confidence;
        
        if (mainVariation === 0) {
            console.log('⚠️ Sistema principal sem variação - dupla projeção não calculada');
            return;
        }
        ...
        if (!currentPrice || currentPrice <= 0) {
            console.log('⚠️ Preço WINFUT não inserido - dupla projeção não calculada');
            return;
        }
        
        // PROJEÇÃO 1: Saldo Final Completo (sistema principal)
        const projection1 = currentPrice * (1 + mainVariation / 100);
        const pointsChange1 = projection1 - currentPrice;
        
        // PROJEÇÃO 2: Usar o MESMO valor do painel "Saldo Remanescente ADRs After Market" com conversão realista
        const saldoADRsData = calculateSaldoRemanescenteADRs(); // ✅ CORRIGIDO: Usar função principal
        // ✅ APLICAR MESMA LÓGICA REALISTA: converter percentual para decimal (dividir por 100)
        const adrsVariationRealista = saldoADRsData.saldo / 100; // 8,0000% → 0,08 (realista)
        const projection2 = currentPrice * (1 + adrsVariationRealista / 100);
        const pointsChange2 = projection2 - currentPrice;
        
        // Calcular região média
        const averageProjection = (projection1 + projection2) / 2;
        const regionRange = Math.abs(projection1 - projection2) / 2;
        
        this.dualProjectionData = {
            currentPrice: currentPrice,
            projection1: projection1,
            projection2: projection2,
            pointsChange1: pointsChange1,
            pointsChange2: pointsChange2,
            averageProjection: averageProjection,
            regionRange: regionRange,
            confidence: mainConfidence,
            saldoADRsData: saldoADRsData
        };
        
        console.log('🎯 DUPLA PROJEÇÃO calculada (ADICIONAL):', {
            fechamento: currentPrice,
            projecao1_saldoFinal: projection1.toFixed(1),
            projecao2_adrsAfter: projection2.toFixed(1),
            regiao_media: averageProjection.toFixed(1),
            range: regionRange.toFixed(1)
        });
        
        this.updateDualProjectionDisplay();
        this.generateRegionRecommendation();
    }
    
    // Função ADICIONAL: Atualizar interface de dupla projeção
    updateDualProjectionDisplay() {
        if (!this.dualProjectionData) return;
        
        const { projection1, projection2, averageProjection, regionRange, pointsChange1, pointsChange2 } = this.dualProjectionData;
        
        // Atualizar elementos da interface adicional (se existirem)
        const dualIndicator = document.getElementById('winfut-dual-indicator');
        if (dualIndicator) {
            dualIndicator.innerHTML = `
                <div class="dual-projection-display">
                    <div class="projection-row">
                        <span class="proj-label">Saldo Final:</span>
                        <span class="proj-value ${pointsChange1 >= 0 ? 'positive' : 'negative'}">${projection1.toFixed(1)}</span>
                        <span class="proj-change">(${pointsChange1 >= 0 ? '+' : ''}${pointsChange1.toFixed(1)})</span>
                    </div>
                    <div class="projection-separator">⟷</div>
                    <div class="projection-row">
                        <span class="proj-label">ADRs After:</span>
                        <span class="proj-value ${pointsChange2 >= 0 ? 'positive' : 'negative'}">${projection2.toFixed(1)}</span>
                        <span class="proj-change">(${pointsChange2 >= 0 ? '+' : ''}${pointsChange2.toFixed(1)})</span>
                    </div>
                    <div class="projection-average">
                        <span class="avg-label">Região Média:</span>
                        <span class="avg-value">${averageProjection.toFixed(1)}</span>
                        <span class="avg-range">±${regionRange.toFixed(1)}</span>
                    </div>
                </div>
            `;
        }
    ...
    // Função ADICIONAL: Gerar recomendação com regiões médias
    generateRegionRecommendation() {
        if (!this.dualProjectionData) return;
        
        const { averageProjection, regionRange, pointsChange1, pointsChange2, confidence } = this.dualProjectionData;
        const upperRegion = averageProjection + regionRange;
        const lowerRegion = averageProjection - regionRange;
        const regionSize = regionRange * 2;
        
        const regionElement = document.getElementById('winfut-region-recommendation');
        if (!regionElement) return;
        
        let recommendation = '';
        
        if (regionSize >= 50 && regionSize <= 200) {
            // REGIÃO COMPACTA - Movimento médio normal (50-200 pontos)
            recommendation = `
            🎯 REGIÃO DE CONFLUÊNCIA ALTA: ${lowerRegion.toFixed(0)} - ${upperRegion.toFixed(0)} pontos (±${regionRange.toFixed(0)} pontos)
            
            📊 ANÁLISE DAS DUAS PROJEÇÕES:
            • Saldo Final: ${(this.dualProjectionData.currentPrice + pointsChange1).toFixed(1)} pontos (${pointsChange1 >= 0 ? '+' : ''}${pointsChange1.toFixed(1)})
            • ADRs After: ${(this.dualProjectionData.currentPrice + pointsChange2).toFixed(1)} pontos (${pointsChange2 >= 0 ? '+' : ''}${pointsChange2.toFixed(1)})
            • Região Média: ${averageProjection.toFixed(1)} ±${regionRange.toFixed(1)} pontos
            
            🎯 ESTRATÉGIA DE REGIÃO:
            • SUPORTE: ${lowerRegion.toFixed(0)} pontos - Entrada de compra se testar
            • RESISTÊNCIA: ${upperRegion.toFixed(0)} pontos - Entrada de venda se testar
            • CENTRO: ${averageProjection.toFixed(0)} pontos - Zona de equilíbrio
            
            ⏰ TIMING: Aguardar abertura e confirmar em qual região o mercado se posiciona
            📈 CONFLUÊNCIA: Alta concordância entre projeções - Sinal mais confiável
            `;
        } else if (regionSize > 200 && regionSize <= 500) {
            // REGIÃO MODERADA - Movimento médio-alto (200-500 pontos)
            recommendation = `
            ⚠️ REGIÃO DE CONFLUÊNCIA MODERADA: ${lowerRegion.toFixed(0)} - ${upperRegion.toFixed(0)} pontos (±${regionRange.toFixed(0)} pontos)
            ...
            🎯 ESTRATÉGIA DE REGIÃO:
            • ZONA INFERIOR: ${lowerRegion.toFixed(0)} pontos - Possível suporte
            • ZONA SUPERIOR: ${upperRegion.toFixed(0)} pontos - Possível resistência
            • CENTRO: ${averageProjection.toFixed(0)} pontos - Aguardar definição
            
            ⏰ TIMING: Aguardar 3º-7º candle para confirmar direção
            📊 CONFLUÊNCIA: Moderada - Aguardar fluxo interno como motorista
            `;
        } else if (regionSize > 500 && regionSize <= 1000) {
            // REGIÃO AMPLA - Movimento alto (500-1000 pontos)
            recommendation = `
            🔄 REGIÃO DE DIVERGÊNCIA: ${lowerRegion.toFixed(0)} - ${upperRegion.toFixed(0)} pontos (±${regionRange.toFixed(0)} pontos)
            ...
            🎯 ESTRATÉGIA DE REGIÃO:
            • ZONA AMPLA: Grande diferença entre projeções
            • AGUARDAR: Mercado interno como motorista principal
            • OBSERVAR: Qual projeção o mercado vai validar
            
            ⏰ TIMING: Aguardar definição clara após abertura
            ⚠️ CONFLUÊNCIA: Baixa - Priorizar fluxo e confluência técnica
            📊 MOVIMENTO: Região ampla - aguardar definição de direção
            `;
        } else {
            // REGIÃO EXTREMA - Movimento extremo (> 1000 pontos)
            recommendation = `
            🚨 REGIÃO DE DIVERGÊNCIA EXTREMA: ${lowerRegion.toFixed(0)} - ${upperRegion.toFixed(0)} pontos (±${regionRange.toFixed(0)} pontos)
            ...
            🎯 ESTRATÉGIA DE REGIÃO (MOVIMENTO EXTREMO):
            • ZONA MUITO AMPLA: Divergência extrema entre projeções
            • AGUARDAR: Definição clara do mercado interno
            • CUIDADO: Alta volatilidade esperada
            
            ⏰ TIMING: Aguardar estabilização e definição clara
            ⚠️ CONFLUÊNCIA: Muito baixa - Priorizar gestão de risco
            📊 MOVIMENTO: Região extrema - máxima cautela
            `;
        }
        
        regionElement.innerHTML = recommendation;
    }
    
    // Função para habilitar/desabilitar o sistema adicional
    toggleDualProjection() {
        this.isEnabled = !this.isEnabled;
        console.log(`🔄 Sistema de Dupla Projeção ${this.isEnabled ? 'HABILITADO' : 'DESABILITADO'}`);
        
        // Atualizar botão
        const toggleButton = document.getElementById('dual-projection-toggle');
        if (toggleButton) {
            toggleButton.textContent = this.isEnabled ? 'Desativar Dupla Projeção' : 'Ativar Dupla Projeção';
            toggleButton.className = this.isEnabled ? 'btn btn-sm btn-primary' : 'btn btn-sm btn-outline-primary';
        }
        ...
        if (this.isEnabled) {
            if (dualIndicator) dualIndicator.style.display = 'block';
            if (regionElement) regionElement.style.display = 'block';
            this.calculateDualProjection();
        } else {
            if (dualIndicator) {
                dualIndicator.style.display = 'none';
                dualIndicator.innerHTML = '';
            }
            if (regionElement) {
    ...
    // Função para ser chamada automaticamente quando o sistema principal for atualizado
    onMainSystemUpdate() {
        if (this.isEnabled) {
            this.calculateDualProjection();
        }
...
// Instanciar o serviço de projeção WINFUT (ORIGINAL - não modificado)
const winfutProjectionService = new WinfutProjectionService();

// Instanciar a extensão adicional (NOVA - não interfere no original)
const winfutDualProjection = new WinfutDualProjectionExtension();

// ===== INICIALIZAÇÃO DO SISTEMA DE DUPLA PROJEÇÃO =====
// Função para inicializar o sistema adicional quando a página carregar
function initializeDualProjectionSystem() {
    console.log('🚀 Inicializando Sistema de Dupla Projeção WINFUT...');
    
    // Verificar se os elementos da interface existem
    const dualSection = document.querySelector('.winfut-dual-section');
    const toggleButton = document.getElementById('dual-projection-toggle');
    
    if (!dualSection || !toggleButton) {
        console.warn('⚠️ Elementos da interface de dupla projeção não encontrados');
        return;
    }
    
    console.log('✅ Sistema de Dupla Projeção WINFUT inicializado com sucesso!');
    console.log('💡 Use o botão "Ativar Dupla Projeção" para habilitar as funcionalidades adicionais');
}
// ===== CONTROLE DO WIDGET WINFUT OPCIONAL =====
// Função para alternar visibilidade do widget WINFUT
function toggleWinfutWidget() {
    const winfutSection = document.getElementById('winfut-section');
    const toggleButton = document.getElementById('toggleWinfutWidget');
    
    if (!winfutSection || !toggleButton) {
        console.error('❌ Elementos do widget WINFUT não encontrados');
        return;
    }
    
    const isVisible = winfutSection.style.display !== 'none';
    
    if (isVisible) {
        // Ocultar widget
        winfutSection.style.display = 'none';
        toggleButton.textContent = '📈 Ativar Projeção';
        toggleButton.className = 'btn btn-sm btn-success';
        console.log('🔄 Widget WINFUT ocultado - espaço economizado');
    } else {
        // Mostrar widget
        winfutSection.style.display = 'block';
        toggleButton.textContent = '✖ Ocultar Projeção';
        toggleButton.className = 'btn btn-sm btn-warning';
        console.log('📈 Widget WINFUT ativado');
        
        // Scroll suave para o widget (abre para baixo)
        setTimeout(() => {
            winfutSection.scrollIntoView({ behavior: 'smooth', block: 'end' });
        }, 100);
    }
}

// Função para inicializar o estado do widget WINFUT
function initializeWinfutWidget() {
    const winfutSection = document.getElementById('winfut-section');
    const toggleButton = document.getElementById('toggleWinfutWidget');
    
    if (winfutSection && toggleButton) {
        // Iniciar com widget oculto para economizar espaço
        winfutSection.style.display = 'none';
        toggleButton.textContent = '📈 Ativar Projeção';
        toggleButton.className = 'btn btn-sm btn-success';
        console.log('✅ Widget WINFUT inicializado como oculto (economiza espaço)');
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        initializeDualProjectionSystem();
        initializeWinfutWidget();
    }, 500);
});
...
function calculateAdvancedWinfutRegions(saldo, confidence, vixLevel) {
    ...
    // 5. ✅ MELHORADO: Calcular multiplicadores inteligentes baseados na força do sinal ADRs
    const signalStrength = Math.abs(saldo); // Força do sinal ADRs
    let optimisticMultiplier, pessimisticMultiplier;
    
    // ✅ NOVO: Multiplicadores baseados na força do sinal + regime de volatilidade
    let baseMultiplier;
    if (signalStrength > 2.0) {
        baseMultiplier = 0.015; // Sinal forte - movimentos maiores (1.5%)
    } else if (signalStrength > 1.0) {
        baseMultiplier = 0.008; // Sinal médio - movimentos moderados (0.8%)
    } else if (signalStrength > 0.5) {
        baseMultiplier = 0.005; // Sinal fraco - movimentos pequenos (0.5%)
    } else {
        baseMultiplier = 0.003; // Sinal muito fraco - movimentos mínimos (0.3%)
    }
    
    // Aplicar regime de volatilidade ao multiplicador base
    const adjustedMultiplier = baseMultiplier * volatilityMultiplier;
    
    optimisticMultiplier = 1 + adjustedMultiplier;
    pessimisticMultiplier = 1 - adjustedMultiplier;
    
    console.log('🎯 Multiplicadores inteligentes:', {
        signalStrength: signalStrength.toFixed(3),
        baseMultiplier: (baseMultiplier * 100).toFixed(1) + '%',
        volatilityMultiplier: volatilityMultiplier.toFixed(1) + 'x',
        adjustedMultiplier: (adjustedMultiplier * 100).toFixed(1) + '%',
        optimistic: ((optimisticMultiplier - 1) * 100).toFixed(1) + '%',
        pessimistic: ((pessimisticMultiplier - 1) * 100).toFixed(1) + '%'
    });
    
    // ✅ NOVO: Calcular probabilidades dinâmicas baseadas na confiança e força do sinal
    const baseConfidence = confidence / 100; // Converter para decimal (0.63 = 63%)
    const adrsDirection = saldo > 0 ? 'positive' : 'negative';
    
    let centralProbability, optimisticProbability, pessimisticProbability;
    
    // Ajustar probabilidades baseado na confiança
    if (baseConfidence > 0.8) {
        // Alta confiança - mais peso na região central
        centralProbability = 0.60;
        optimisticProbability = 0.25;
        pessimisticProbability = 0.15;
    } else if (baseConfidence > 0.6) {
        // Confiança média - distribuição equilibrada
        centralProbability = 0.50;
        optimisticProbability = 0.30;
        pessimisticProbability = 0.20;
    } else if (baseConfidence > 0.4) {
        // Confiança baixa - mais incerteza
        centralProbability = 0.40;
        optimisticProbability = 0.35;
        pessimisticProbability = 0.25;
    } else {
        // Muito baixa confiança - distribuição uniforme
        centralProbability = 0.35;
        optimisticProbability = 0.35;
        pessimisticProbability = 0.30;
    }
    
    // ✅ NOVO: Ajustar baseado na direção dos ADRs
    if (adrsDirection === 'positive' && signalStrength > 1.0) {
        // ADRs positivos e fortes - favorecer região otimista
        optimisticProbability += 0.05;
        pessimisticProbability -= 0.05;
    } else if (adrsDirection === 'negative' && signalStrength > 1.0) {
        // ADRs negativos e fortes - favorecer região pessimista
        optimisticProbability -= 0.05;
        pessimisticProbability += 0.05;
    }
    
    console.log('🎯 Probabilidades dinâmicas:', {
        baseConfidence: (baseConfidence * 100).toFixed(1) + '%',
        adrsDirection: adrsDirection,
        signalStrength: signalStrength.toFixed(3),
        probabilities: {
            central: (centralProbability * 100).toFixed(0) + '%',
            optimistic: (optimisticProbability * 100).toFixed(0) + '%',
            pessimistic: (pessimisticProbability * 100).toFixed(0) + '%'
        }
    });
    ...
    const regions = {
        ...
        central: {
            ...
            probability: centralProbability,
    ...
    // 6. ✅ MELHORADO: Aplicar faixas adaptativas baseadas na volatilidade e confiança
    Object.keys(regions).forEach(key => {
        const region = regions[key];
        
        // ✅ NOVO: Faixa baseada na confiança e volatilidade
        let baseVolatility;
        if (key === 'central') {
            // Região central - menor volatilidade (mais precisa)
            baseVolatility = 0.008; // 0.8% base
        } else if (key === 'optimistic') {
            // Região otimista - volatilidade média
            baseVolatility = 0.012; // 1.2% base
        } else {
            // Região pessimista - maior volatilidade (mais incerta)
            baseVolatility = 0.015; // 1.5% base
        }
        
        // Ajustar baseado na confiança
        const confidenceAdjustment = Math.max(0.5, baseConfidence); // Mínimo 50% da faixa
        const adjustedVolatility = baseVolatility * volatilityMultiplier * confidenceAdjustment;
        
        region.lowerBound = region.center - (region.center * adjustedVolatility);
        region.upperBound = region.center + (region.center * adjustedVolatility);
        region.volatility = adjustedVolatility;
        region.baseVolatility = baseVolatility;
        region.confidenceAdjustment = confidenceAdjustment;
        
        console.log(`🎯 Faixa ${key}:`, {
            center: region.center.toFixed(0),
            baseVolatility: (baseVolatility * 100).toFixed(1) + '%',
            confidenceAdjustment: (confidenceAdjustment * 100).toFixed(0) + '%',
            adjustedVolatility: (adjustedVolatility * 100).toFixed(1) + '%',
            range: `${region.lowerBound.toFixed(0)}-${region.upperBound.toFixed(0)}`
        });
    });
    
    console.log('✅ Regiões WINFUT MELHORADAS calculadas:', {
        regime,
        timeMultiplier,
        ...
        regions: Object.keys(regions).map(k => ({
            ...
            range: `${regions[k].lowerBound.toFixed(0)}-${regions[k].upperBound.toFixed(0)}`,
            volatility: (regions[k].volatility * 100).toFixed(1) + '%'
        }))
    });
...
// Função para inicializar ScrollReveal nos cards de cotações
function initializeScrollReveal() {
    // Verificar se ScrollReveal está disponível
    if (typeof ScrollReveal === 'undefined') {
        console.warn('ScrollReveal não está disponível. Carregando...');
        setTimeout(initializeScrollReveal, 100);
        return;
    }

    console.log('🎬 Inicializando ScrollReveal para cards de cotações...');

    // Configuração principal do ScrollReveal
    ...
    // Efeito para cards da primeira linha (macro indicators)
    sr.reveal('#quote-vix', {
        delay: 100,
        origin: 'top',
        scale: 0.9,
    ...
    // Efeito para cards da segunda linha (ADRs brasileiros) - efeito mais dinâmico
    sr.reveal('#quote-vale', {
        delay: 500,
        origin: 'left',
        scale: 0.8,
        opacity: 0,
    ...
    console.log('✅ ScrollReveal configurado com sucesso para todos os cards de cotações!');
}