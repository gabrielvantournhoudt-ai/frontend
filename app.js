...
class WinfutDualProjectionExtension {
    ...
    // Função ADICIONAL: Calcular apenas Saldo Remanescente ADRs After Market
    calculateSaldoRemanescenteADRsOnly() {
        let saldoRemanescente = 0;
        let adrsComDados = 0;
        
        // Calcular ADRs principais
        if (typeof adrsData !== 'undefined' && adrsData.principais) {
            adrsData.principais.forEach(adr => {
                const afterInput = document.getElementById(`after-${adr.ticker}`);
                if (afterInput && afterInput.value !== '') {
                    const afterValue = parseFloat(afterInput.value) || 0;
        ...
        // Calcular ADRs secundários (se habilitado)
        if (typeof appState !== 'undefined' && appState.incluirSecundarios && adrsData.secundarios) {
            adrsData.secundarios.forEach(adr => {
                const afterInput = document.getElementById(`after-${adr.ticker}`);
                if (afterInput && afterInput.value !== '') {
                    const afterValue = parseFloat(afterInput.value) || 0;
    ...
    calculateDualProjection() {
        ...
        // PROJEÇÃO 2: Apenas Saldo Remanescente ADRs After Market
        const saldoADRsData = this.calculateSaldoRemanescenteADRsOnly();
        const projection2 = currentPrice * (1 + saldoADRsData.saldo / 100);
